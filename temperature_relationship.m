...
while(1)
    ...
    temp1arr(TestVolCounter*test_step-test_step+1:TestVolCounter*test_step) = temp2;
    ...
    if(TestVolCounter == store_count/test_step)
        %% 创建记录文件
        filename = strcat(pwd,strcat(strcat('\data\',['Longtermstabletest_No',num2str(TestCounter),'_',datestr(now,30)]),'.xls'));
        ...
        xlswrite(filename,{'采样时间'},1,'A1:A1');
        xlswrite(filename,{'ADC1Channel1'},1,'B1:B1');
        xlswrite(filename,{'ADC1Channel2'},1,'C1:C1');
        xlswrite(filename,{'ADC2Channel1'},1,'D1:D1');
        xlswrite(filename,{'ADC2Channel2'},1,'E1:E1');
        xlswrite(filename,time_arr(:),1,'A2:A100001');
        xlswrite(filename,database1(:),1,'B2:B100001');
        xlswrite(filename,database2(:),1,'C2:C100001');
        xlswrite(filename,database3(:),1,'D2:D100001');
        xlswrite(filename,database4(:),1,'E2:E100001');
...
%%
subplot(3,2,3);
plot(time_arr(1:1:TestCounter), database1(1:1:TestCounter), 'r.', 'MarkerSize', 6);
title('通道1电压');
ylabel('电压(V)');
hold on;
datetick('x', 0);
subplot(3,2,5);
plot(time_arr(1:1:TestCounter), database2(1:1:TestCounter), 'g.', 'MarkerSize', 6);
title('通道2电压');
ylabel('电压(V)');
xlabel('采样时间');
hold on;
datetick('x', 0)
subplot(3,2,4);
plot(time_arr(1:1:TestCounter), database3(1:1:TestCounter), 'b.', 'MarkerSize', 6);
title('通道3电压');
hold on;
datetick('x', 0);
subplot(3,2,6);
plot(time_arr(1:1:TestCounter), database4(1:1:TestCounter), 'k.', 'MarkerSize', 6);
title('通道4电压');
xlabel('采样时间');
hold on;
datetick('x', 0)
...
%      hold on;
subplot(3,2,1);
plot(time_arr(1:1:TestCounter), temp1arr(1:1:TestCounter), 'cs', 'MarkerSize', 6);
title('芯片1温度');
ylabel('温度（℃）');
hold on;
datetick('x', 0);
subplot(3,2,2);
plot(time_arr(1:1:TestCounter), temp2arr(1:1:TestCounter), 'md', 'MarkerSize', 6);
title('芯片2温度');
hold on;
datetick('x', 0);
