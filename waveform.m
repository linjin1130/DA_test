...
classdef waveform
    ...
    methods(Static = true)
        ...
        function seq = generate_trig_seq(count,delay)
            ...
            function_ctrl = 64; %128 表示停止， 64表示触发，32表示计数
            counter_ctrl = 4;   %触发输出时表示触发执行次数，计数输出时表示计数值，连续输出时无效
            trigger_ctrl = 0;       %48-55位
            length_wave  = 12500;   %16-31位,输出波形长度
            address_wave = 0;  
            seq(4*1-1) = counter_ctrl;
            seq(4*1-0) = function_ctrl*256 + trigger_ctrl;
            seq(4*1-3) = address_wave;
            seq(4*1-2) = length_wave;
            function_ctrl = 128+64;
            for  k = 8:4:16384 
                seq(k-1) = counter_ctrl;
                seq(k-0) = function_ctrl*256 + trigger_ctrl;
                seq(k-3) = address_wave;
                seq(k-2) = length_wave;
            end
        end
        
        function  total_point = best_fit_count(fre)
            %SETDACFREQUENCY 在2G采样率和最大波形深度为memory_size条件下生成给定频率波形
            % 在固定采样率和有限长波形条件下生成一个频率误差最小的波形
            memory_size = 100000;
            sample_rate = 2e9;
            period_point = 1/fre*sample_rate;
            err = ones(1,floor(memory_size/period_point));
            for k = 1:floor(memory_size/period_point)
